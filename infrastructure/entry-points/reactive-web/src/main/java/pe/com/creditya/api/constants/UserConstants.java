package pe.com.creditya.api.constants;

public class UserConstants {
    public static final String LOGGER_START = "Request recibido para registrar usuario: {}";
    public static final String LOGGER_MAP = "Transformando UserRequest a User: {}";
    public static final String LOGGER_SAVE_SUCCESS = "Usuario registrado con éxito: {}";
    public static final String LOGGER_SAVE_FAIL = "Error al registrar usuario: {}";
    public static final String NOT_FOUNT_RESOURCE = "El recurso solicitado no existe: ";
    public static final String LOGGER_VALIDATION_FAIL = "Error en la validacion del request: {} | Violations: {}";
    public static final String LOGGER_VALIDATION_SUCCESS = "Validacion exitoso del request: {}";

}
